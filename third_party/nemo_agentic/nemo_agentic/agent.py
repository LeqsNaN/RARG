...
class Agent:
    ...
    async def conclude_task(self, query: str, task_info: Optional[Any] = None) -> Dict[str, Any]:
        ...
        has_successful_final_results = False
        if self.config.end_tool in self.tool_map:
            end_tool = self.tool_map[self.config.end_tool]
            correct_val = getattr(end_tool, "correct_call_return_value", None)
            if isinstance(correct_val, str):
                for msg in reversed(self.message_history):
                    if msg.get("role") != "tool" or msg.get("name") != self.config.end_tool:
                        continue
                    content = msg.get("content", [])
                    if isinstance(content, list) and content and content[0].get("text") == correct_val:
                        has_successful_final_results = True
                    break

        if has_successful_final_results:
            return output_artifacts
        ...
        if isinstance(rrf_res, dict) and len(rrf_res) != 0:
            return output_artifacts

        selection_topk_list = self.config.selection_topk_list
        if len(selection_topk_list) == 0 and self.is_last_msg_error():
            selection_topk_list = [10]

        if len(selection_topk_list) != 0 and len(selection_input) != 0:
            for _ in range(2):
